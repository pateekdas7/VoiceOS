# production environment -- full Multi-AZ topology (V7 Ch2).

module "voiceos" {
  source = "../../"

  environment = var.environment
  region      = var.region
  tags        = var.tags

  availability_zones = ["ap-south-1a", "ap-south-1b"]

  cpu_node_instance_type    = "m6i.xlarge"
  gpu_node_instance_type    = "g5.2xlarge"
  data_node_instance_type   = "m6i.xlarge"
  system_node_instance_type = "m6i.large"

  cpu_node_desired_count    = 3
  gpu_node_desired_count    = 2
  data_node_desired_count   = 3
  system_node_desired_count = 3

  postgres_instance_class       = "db.r6g.xlarge"
  postgres_multi_az             = true
  postgres_allocated_storage_gb = 500

  redis_node_type = "cache.r6g.xlarge"
  redis_multi_az  = true

  mongodb_ami_id = var.mongodb_ami_id
}
