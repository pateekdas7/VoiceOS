...
**Path-A Runtime Consolidation Status (2026-07-25):** 🟡 **Phases 1–6 COMPLETE, Phase 7 IN PROGRESS (blocked on GPU node connectivity).** Triggered by a pre-Call-002 architecture audit that found two disconnected implementations — the designed architecture (unreachable by any live call) and `conv_server.py` (a standalone script with its own duplicated logic that actually took Call-001). `ConversationEngine` is now the sole production runtime with a real Twilio WS entrypoint (Phase 4), real STT/PTP-persistence wiring (Phase 3/5), and the ported/redesigned scripted-response FSM (`DialogueResponseEngine`, Phase 6a-6g) wired in as the primary reply path — 113 new tests, full regression 2194 passed/73 skipped/0 failed, `check_boundaries.py` clean. Phase 7 (a real dry-run call through the full stack) is blocked: the GPU node (`62.169.159.20`) became unreachable on all ports including SSH mid-session after a provider security-group change, despite its `voiceos-{stt,llm,tts}` services being confirmed healthy and correctly bound moments earlier — `tcpdump` on the GPU node's own NIC captured zero inbound SYN packets from two independent external IPs during the outage, proving the block is enforced upstream at the provider's network edge, not by anything on the VM. Per explicit user direction, further live GPU validation is deferred rather than worked around. Phase 8 (retire `conv_server.py`) and Call-002 remain pending Phase 7. Full detail in CHANGELOG.md's "Path-A Runtime Consolidation, Phases 1–7" entry.
