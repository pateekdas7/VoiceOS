...
**Current Phase:** Sprint-029 (Founder Validation) — Phase 2 pending real call transcripts. Phase 3 Sales Production Action Layer shipped (2026-09-10).
**Current Epic:** E8 — Founder Validation
**Previous Epic:** E7 — Production Alpha (Sprint-028 AC-1 PASS 2026-07-12)
**Current Sprint:** Sprint-029
**Sprint Status:** 🟢 Sprint-028 AC-1 PASS (Run F: first_audio p95=995ms). Phase 3 Sales Production Action Layer complete (PipelineTransitionEngine, SalesProductionActionDispatcher, RelationshipContextBuilder, PostCallSummaryRepository, generate_post_call_summary, ConversationEngine wiring, API endpoint, 34 tests). CPU node migrating from cloud VPS to phone 2 (Android/Termux) — setup script at `deployment/phone/setup_phone2.sh`.
...
**Repository Version:** v2.0.34 (BFF Phase 10 — Request Lifecycle Hardening — 2026-09-17)
...
| Phases 11–16 | TBD — pending user direction | ⏳ NOT STARTED | — |
...
**Immediate (phone 2 setup):**
1. Complete `bash setup_phone2.sh` on phone 2 (currently installing packages)
2. Fill in `deployment/cpu/.env` — Twilio creds + Kaggle tunnel URLs
3. Start Kaggle GPU kernel → copy 3 tunnel URLs to `.env`
4. Run smoke-test: `python deployment/cpu/app.py --smoke-test`
5. Place a live Twilio call end-to-end through phone 2 CPU + Kaggle T4

**Sprint-029 Phase 2:** Founder validation with real call transcripts (pending live calls)

**Sprint-030:** Pilot deployment

**BFF Phases 11–16:** Pending user direction — see GPU_DEPLOYMENT_CHECKLIST.md for deferred tests

**Phase 3 Sales (done 2026-09-10):**
- ✅ PipelineTransitionEngine (FSM validation)
- ✅ SalesProductionActionDispatcher (SCHEDULE_FOLLOWUP + HUMAN_HANDOFF)
- ✅ RelationshipContextBuilder (historical prompt injection)
- ✅ PostCallSummaryRepository (tenant-isolated Postgres)
- ✅ generate_post_call_summary (deterministic serialization)
- ✅ ConversationEngine wired (Steps 2b/3b/4b/5b + end_call summary)
- ✅ GET /calls/{call_id}/summary API endpoint
- ✅ 34 tests pass (3 skipped cffi/Termux)
