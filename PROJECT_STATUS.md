...
**Path-A Runtime Consolidation Status (2026-07-25):** ✅ **Phases 1–8 COMPLETE. `ConversationEngine` is the sole production runtime; `conv_server.py` retired (archived to `evaluation/founder-validation/archive/`). Call-002 proposed, pending user authorization.** Triggered by a pre-Call-002 architecture audit that found two disconnected implementations — the designed architecture (unreachable by any live call) and `conv_server.py` (a standalone script with its own duplicated logic that actually took Call-001). `ConversationEngine` is now the sole production runtime with a real Twilio WS entrypoint (Phase 4), real STT/PTP-persistence wiring (Phase 3/5), and the ported/redesigned scripted-response FSM (`DialogueResponseEngine`, Phase 6a-6g) wired in as the primary reply path. Phase 7's live multi-turn dry run against real GPU/Postgres infra passed after diagnosing and fixing two real defects: (1) a Path MTU black-hole on the CPU→GPU network path — the first code path in this project's history to ever exercise sustained cross-host streaming inference traffic (Call-001 ran entirely on the GPU node itself via Twilio-native STT/TTS, never touching this path) — fixed via client-side TCP MSS clamping, persisted via a systemd unit and verified across a real CPU node
reboot (TT-028); (2) a real pre-existing RI-3 crash risk — `PlaybackScheduler`'s queue was never drained by `CallOrchestrator`, so any real call with >~43s of cumulative AI speech would have crashed outright — fixed by draining it per turn. 117 new tests total, full regression 2197 passed/73 skipped/0 failed, `check_boundaries.py` clean. Phase 8 retired `conv_server.py` (and its sibling `empathy_directive.py`) to `evaluation/founder-validation/archive/` after an independent audit confirmed no live traffic could reach it — no systemd/Compose/CI job ever started it, the GPU node hosting it was rebuilt without it, its Call-001 webhook was an ephemeral tunnel URL, and no test imports it; full regression unchanged (2197 passed/73 skipped) after the move. Full detail in CHANGELOG.md's "Path-A Runtime Consolidation, Phases 1–8" entry. **Post-Phase-8 same day:** before scheduling Call-002 per explicit user requirement, TT-028 was fully resolved (systemd-unit persistence, verified across a real CPU node reboot) and Call-002 was scoped as a full production-component validation rather than a Call-001 repeat — which required building a real, live LLM-fallback trigger condition (`DialogueTurnOutput.needs_llm_fallback`, two consecutive unclassified turns) since the LLM path had been structurally unreachable at runtime despite being "wired." Validating that trigger against real GPU/LLM infra (`scripts/path_a_llm_fallback_validation.py`) surfaced and led to fixing three real defects in one session: RegisterGuard was never applied to the LLM streaming path at all (fixed via `TrueStreamingPipeline`); the masculine-grammar blocklist only covered Devanagari phrases (replaced with systematic Roman+Devanagari suffix-rule regexes per explicit direction, not phrase enumeration); and both system-wide "safe fallback" constants were themselves grammatically masculine (a pre-Kavya-persona Sprint-018 default), which is what made the RegisterGuard fix initially look broken on retest. 19 new tests; full regression 2223 passed/73 skipped/0 failed, `check_boundaries.py` clean throughout. Full detail in CHANGELOG.md's "Post-Phase-8 — Call-002 scoping follow-ups" entry.
**Project Status:** 🟢 Sprint-028 PASS — AC-1 (first_audio p95 ≤ 1500ms) **PASS** (Run F: **995ms**, 505ms below gate). Run E's 1553ms FAIL was a vLLM CUDA-graph state artifact from prior session restarts; Run F on a clean server at the same 0.45 GPU util delivers LLM p50=74ms. Pre-Sprint-029 Phase 2 audit complete 2026-07-12. Load test, chaos GPU failover, canary deferred (fleet constraint). Milestones M-3 through M-6 complete; M-7 (Production Alpha) PASS; M-8 (Founder Validation) NOT yet achieved.
...
**Sprint Status:** 🟢 Sprint-028 AC-1 PASS (Run F: first_audio p95=995ms, 2026-07-12). Load test FAIL (single L4), chaos GPU failover FAIL (no fleet), canary NOT EXECUTED — these remain deferred to fleet deployment. All offline ACs: ADR-004 APPROVED (AC-8 PASS), security fixes (PEN-005/006/007/009 FIXED), compliance code audit (AC-5 PASS). Sprint-029 Phase 1 (offline evaluation suite) complete.
...
**Repository Version:** v2.0.28 (Sprint-028 — Performance Validation, Load Testing, Pen Test & Production Alpha Deploy — PARTIAL, 2026-07-12 updates in progress)
**Current Branch:** claude/ssh-gpu-cpu-servers-y99fib
**Last Commit:** Path-A Runtime Consolidation Phase 6-close (2026-07-25) — `DialogueResponseEngine` wired into the composition root (`deployment/cpu/app.py`), completing Phases 1-6 of the 8-phase consolidation plan. See CHANGELOG.md's "Path-A Runtime Consolidation, Phases 1–7" entry for the full 6a-6g breakdown. Previous: Sprint-028 Phase 2 (2026-07-12) — ADR-004 approved (TTS budget 250ms→750ms); PEN-005/006/007/009 security fixes deployed; compliance code audit (AUD-002/003 confirmed correct); GPU node restored to 217.18.55.120; bootstrap.sh ffmpeg fix; Run D latency data; evaluation reports updated. See CHANGELOG.md's Sprint-028 Phase 2 Updates entry.
...
**Sprint-028 (AC-1 PASSED, 2026-07-12):**
1. ✅ Run F complete — first_audio p95=995ms PASS, LLM TTFT p50=74ms PASS
2. ✅ latency-report.md updated with Run F results and PASS sign-off
3. ✅ PROJECT_STATUS.md updated to PASS
4. Update CHANGELOG.md with Run F entry
5. Commit all changes to `claude/ssh-gpu-cpu-servers-y99fib`

**Sprint-029 Phase 1 (complete):** Offline founder validation tooling — `tests/ai_eval/founder_validation_suite.py`, synthetic fixtures, evaluation report template — all done. Phase 2 pending real call transcripts.
