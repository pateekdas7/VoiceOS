...
function evaluateWorkingHours(date,timeZone,policy={}) {
  ...
  const hour=Number(p.hour), start=policy.dailyStartHour==null?9:Number(policy.dailyStartHour), end=policy.dailyEndHour==null?18:Number(policy.dailyEndHour);
  const inWindow=start<=end?hour>=start&&hour<=end:hour>=start||hour<=end;
  return {allowed:inWindow,reason:inWindow?'within_window':'outside_calling_window',localDate,localHour:hour,window:start+'-'+end};
}
