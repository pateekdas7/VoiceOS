"""
Build v5 kernel source: hard male-cluster token blacklist.

Strategy: run 50 unseeded generations on the highest-drift text (P02),
collect all audio token IDs from male-voiced frames only,
build a frequency table, take top-N most male-exclusive tokens,
add them as logit_bias=-100 (effectively -inf after softmax).
This is a token-level hard block on the generation layer.
"""
import os, json
...
V5_DIR = "/data/data/com.termux/files/home/maitri_v5_fix"
os.makedirs(V5_DIR, exist_ok=True)

# Write source
with open(os.path.join(V5_DIR, "source.py"), "w") as f:
    f.write(V5_SRC)

# Write kernel metadata
meta = {
    "id": "prateek777777/voiceos-maitri-voice-switch-fix-v5",
    "title": "VoiceOS Maitri Voice Switch Fix v5",
    "code_file": "voiceos-maitri-voice-switch-fix-v5.ipynb",
    "language": "python",
...
with open(os.path.join(V5_DIR, "kernel-metadata.json"), "w") as f:
    json.dump(meta, f, indent=2)

# Write build script
build_src = '''"""Wrap source.py into single-cell notebook."""
import json, os
HERE = os.path.dirname(os.path.abspath(__file__))
SRC  = os.path.join(HERE, "source.py")
OUT  = os.path.join(HERE, "voiceos-maitri-voice-switch-fix-v5.ipynb")
with open(SRC) as f:
    source = f.read()
...
'''
with open(os.path.join(V5_DIR, "build_ipynb.py"), "w") as f:
    f.write(build_src)

print(f"v5 kernel files written to {V5_DIR}")
