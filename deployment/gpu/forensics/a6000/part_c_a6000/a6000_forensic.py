#!/usr/bin/env python3
"""
A6000 forensic script — Veena kavya drift investigation.

Runs the X4-A/X4-B forensic corpus on an RTX A6000 (SM 8.6 Ampere) to give
an INDEPENDENT NVIDIA comparison point to the T4 (SM 7.5 Turing) baseline.

A6000 is NOT an L4. Do not label these results as L4.

Test matrix (same seed=42, same sampler, same corpus, same tokenizer):
  RUN 1  A6000 BF16     current code (SNAC patched)   SDPA attention
  RUN 2  A6000 FP16     current code (SNAC patched)   SDPA attention
  RUN 3  A6000 BF16     current code (SNAC patched)   EAGER attention
  RUN 4  A6000 BF16     L4-reconstructed code         SDPA attention  (no SNAC patches)
  RUN 5  A6000 FP32     current code (SNAC patched)   SDPA attention  (only if time permits)
...
Output: /root/a6000_forensic_results.json (or ./a6000_forensic_results.json)
"""
...
RESULTS_PATH = os.environ.get("A6000_RESULTS", "/root/a6000_forensic_results.json")
...
def bootstrap() -> None:
    ...
    # If A6000 host already has newer/older versions we DO NOT force downgrade
...
def load_snac_patched(device: str = "cuda") -> Any:
    ...
    # Snake Python fallback (matches T4 patch; on A6000 with NVRTC it slightly
...
def main() -> None:
    ...
    parser.add_argument("--skip-fp32", action="store_true",
                        help="Skip A6000 FP32 run (already known FP32 doesn't help on T4).")
    ...
    print(f"=== A6000 FORENSIC START {time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())}")
    ...
    try:
        ...
        print(f"WARN: unpatched SNAC load failed on A6000: {e}")
        snac_unpatched = None
    ...
    # RUN 1 — A6000 BF16, current code (SNAC patched), SDPA
    try:
        results["runs"]["A6000_BF16_SDPA_current"] = run_config(
            "A6000_BF16_SDPA_current", torch.bfloat16, "sdpa", "patched",
            tokenizer=tokenizer, snac_model=snac_patched,
        )
    except Exception as e:
        import traceback
        results["runs"]["A6000_BF16_SDPA_current"] = {"error": str(e),
                                                       "traceback": traceback.format_exc()}
        print(f"RUN 1 FAILED: {e}")

    # RUN 2 — A6000 FP16, current code (SNAC patched), SDPA
    if not args.skip_fp16:
        try:
            results["runs"]["A6000_FP16_SDPA_current"] = run_config(
                "A6000_FP16_SDPA_current", torch.float16, "sdpa", "patched",
                tokenizer=tokenizer, snac_model=snac_patched,
            )
        except Exception as e:
            import traceback
            results["runs"]["A6000_FP16_SDPA_current"] = {"error": str(e),
                                                           "traceback": traceback.format_exc()}
            print(f"RUN 2 FAILED: {e}")

    # RUN 3 — A6000 BF16, current code, EAGER attention
    if not args.skip_eager:
        try:
            results["runs"]["A6000_BF16_EAGER_current"] = run_config(
                "A6000_BF16_EAGER_current", torch.bfloat16, "eager", "patched",
                tokenizer=tokenizer, snac_model=snac_patched,
            )
        except Exception as e:
            import traceback
            results["runs"]["A6000_BF16_EAGER_current"] = {"error": str(e),
                                                            "traceback": traceback.format_exc()}
            print(f"RUN 3 FAILED: {e}")

    # RUN 4 — A6000 BF16, L4-reconstructed code (SNAC unpatched), SDPA
    if not args.skip_l4_code and snac_unpatched_ok:
        try:
            results["runs"]["A6000_BF16_SDPA_L4code"] = run_config(
                "A6000_BF16_SDPA_L4code", torch.bfloat16, "sdpa", "unpatched",
                tokenizer=tokenizer, snac_model=snac_unpatched,
            )
        except Exception as e:
            import traceback
            results["runs"]["A6000_BF16_SDPA_L4code"] = {"error": str(e),
                                                          "traceback": traceback.format_exc()}
            print(f"RUN 4 FAILED: {e}")

    # RUN 5 — A6000 FP32, current code, SDPA (nice to have)
    if not args.skip_fp32:
        try:
            results["runs"]["A6000_FP32_SDPA_current"] = run_config(
                "A6000_FP32_SDPA_current", torch.float32, "sdpa", "patched",
                tokenizer=tokenizer, snac_model=snac_patched,
            )
        except Exception as e:
            import traceback
            results["runs"]["A6000_FP32_SDPA_current"] = {"error": str(e),
                                                           "traceback": traceback.format_exc()}
    ...
    print(f"=== A6000 FORENSIC DONE {time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())}")
