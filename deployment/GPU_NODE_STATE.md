...
**Status:** ✅ GPU node fully restored + Sprint-029 Founder Evaluator deployed (2026-07-18). Production: STT ✅, LLM ✅ (TTFT 56.8ms, gpu-mem-util 0.32), TTS ✅ (TTFA 918ms ⚠️ TT-010). Evaluator: `voiceos-evaluator.service` (Qwen2.5-Omni-7B, port 8300) enabled + started. CUDA 13.0 compat bridging required (driver 570.195.03 reports CUDA 12.8; `cuda-compat-13-0` + `cuda-nvrtc-13-0` + LD_LIBRARY_PATH in all systemd units). vLLM 0.25.1. `ninja-build` required system-wide for FlashInfer JIT.
