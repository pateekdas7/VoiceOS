...
> **VRAM Budget (resolved — Sprint-009 Phase 2, measured 2026-07-03):** Spec requires 24,576 MB (Whisper 6,144 + Qwen 16,384 + Veena 2,048). Actual GPU: 23,034 MB. Resolved by:
> - Whisper `int8_float16` loads **1,242 MB actual** (6,144 MB is the scheduler reservation for headroom)
> - Qwen FP8 via vLLM `--gpu-memory-utilization 0.55` → **12,628 MB actual** (weights + KV cache). Reduced from 0.70 to 0.55 to make room for Veena BF16.
> - Veena (maya-research/Veena, 3B BF16) + SNAC 24 kHz codec → **7,980 MB actual**
> - **Measured total: 21,850 MB used, 695 MB free** on the 23,034 MB L4 — all three fit with headroom.
...
| **Total (measured)** | | | | **21,850 MB used / 695 MB free** | | |
